...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(ui->visibleDecrypt, &QPushButton::clicked, this, &MainWindow::visibleDecryptClicked, Qt::ConnectionType::DirectConnection);
    connect(ui->visibleEncrypt, &QPushButton::clicked, this, &MainWindow::visibleEncryptClicked, Qt::ConnectionType::DirectConnection);

    visibleDecryptClicked(false);
    visibleEncryptClicked(false);
...
void MainWindow::visibleDecryptClicked(bool checked)
{
    if (checked) {
        ui->decrypSecret->setEchoMode(QLineEdit::EchoMode::Normal);
    } else {
        ui->decrypSecret->setEchoMode(QLineEdit::EchoMode::Password);
    }
}

void MainWindow::visibleEncryptClicked(bool checked)
{
    if (checked) {
        ui->encryptSecret->setEchoMode(QLineEdit::EchoMode::Normal);
    } else {
        ui->encryptSecret->setEchoMode(QLineEdit::EchoMode::Password);
    }
}
